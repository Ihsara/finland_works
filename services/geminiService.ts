...
export const createSystemInstruction = (
  ...
  // 3. Guide Info - USE SUMMARY ONLY for token efficiency
  ...
  **You MUST reply in this language.**
  ...
  <guide info>
  The following are SUMMARIES of the relevant guides to help you understand what the user knows.
  ...
  [NETWORKING OPTIONS]
  Use this EXACT XML structure if the user asks about networking. Do not translate the IDs (values), but the user will see the localized XML generated by the app logic if you output it here.
...
export const sendMessageToGemini = async (
  ...
  // 1. INTERCEPTION LAYER: Check for pre-defined networking paths
  // Check if the newMessage (which is the ID or text) matches a known ID in our dynamic content
  const networkingResponse = getNetworkingResponse(newMessage, language);
  
  if (networkingResponse) {
    // Return structured JSON so the frontend can render text + action button
    // Note: Networking responses still use JSON for now because they trigger app navigation
