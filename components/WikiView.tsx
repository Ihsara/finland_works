...
interface WikiViewProps {
  ...
  // Supporting legacy object prop if needed, or prefer ID
...
const WikiView: React.FC<WikiViewProps> = ({ 
  ...
  // Derived Active Article (Support both ID and Object prop for backward compat/refactor transition)
  const activeArticle = useMemo(() => {
      if (propActiveArticle) return propActiveArticle;
      if (!activeArticleId) return null;
      const all = getAllFlattenedArticles(language);
      return all.find(a => a.id === activeArticleId) || null;
  }, [activeArticleId, propActiveArticle, language]);
  ...
  useEffect(() => {
    ...
    wikiCategories.forEach(cat => {
      // Fix: Check subsections for relevant articles
  ...
  const getCategoryProgress = (cat: WikiCategory) => {
    // Fix: Flatten subsections to get all articles
  ...
  const getCurrentDisplayId = (article: WikiArticle) => {
    // Fix: Iterate subsections to find article
         ...
         {wikiCategories.map((category, catIndex) => {
            ...
            return (
                <div key={category.id} className={`select-none ${!isSidebar ? 'bg-white rounded-xl p-2 border border-transparent hover:border-gray-100 transition-colors' : ''}`}>
                    ...
                    {isOpen && (
                        <div className={`space-y-4 mt-2 ${isSidebar ? 'ml-3 border-l-2 border-gray-200 pl-3' : 'px-2'}`}>
                            {category.subsections.map((sub, subIndex) => (
                                <div key={sub.title} className="space-y-1">
                                    {/* Subsections Headers */}
                                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider px-3 mb-1">
                                        {sub.title}
                                    </h4>
                                    {sub.articles.map((article, artIndex) => {
                                        ...
                                        // Numbering logic is a bit complex with subsections, simple count may be misleading
                                        // Using simple bullet or circle for now
            ...
            <div className="w-full h-full overflow-y-auto bg-gray-50 p-4 md:p-8 animate-in fade-in zoom-in-95 duration-300">
                <div className="max-w-6xl mx-auto">
                     <div className="mb-8 text-center">
                         <h3 className="text-2xl font-bold text-gray-900">{t('wiki_explore_cats')}</h3>
                         <p className="text-gray-600 mt-2">{t('wiki_explore_subtitle')}</p>
                     </div>
             ...
             <div className="w-full h-full overflow-y-auto bg-gray-50 p-4 md:p-8 animate-in slide-in-from-right-4 duration-300">
                <div className="max-w-2xl mx-auto">
                    <div className="mb-6 text-center">
                        <h3 className="text-2xl font-bold text-gray-900">{t('wiki_full_index')}</h3>
                        <p className="text-gray-600 mt-1">{t('wiki_full_index_subtitle')}</p>
                    </div>
                ...
                <div className="w-full p-8 md:p-12 bg-gray-50 border-b border-gray-100">
                    <div className="max-w-4xl mx-auto flex items-center gap-6">
                         ...
                         <div>
                              ...
                              <p className="text-gray-500 font-medium mt-1">{t('wiki_topic_desc')}</p>
                         </div>
                    </div>
                </div>
        ...
        {activeCategory && !activeArticle && !activeTag && (
            <div className="w-full h-full overflow-y-auto bg-white animate-in fade-in slide-in-from-bottom-4 duration-300">
                        ...
                        <div className="text-center md:text-left flex-1">
                            ...
                            <div className="flex items-center justify-center md:justify-start gap-4 text-gray-600 font-medium">
                                <span className="flex items-center gap-1">
                                    ...
                                    {/* Fix: Count articles in subsections */}
                ...
                <div className="max-w-4xl mx-auto p-6 md:p-12 pb-24">
                    ...
                    {/* Fix: Iterate Subsections */}
                    <div className="space-y-10">
                        {activeCategory.subsections.map((sub, subIdx) => (
                            <div key={sub.title}>
                                <h4 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4 px-1 border-b border-gray-100 pb-2">
                                    {sub.title}
                                </h4>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
